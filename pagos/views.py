...
from django.db.models import (
    F, Q, Value, Sum, Count, Avg, DecimalField, CharField, ExpressionWrapper
)
...
from .forms import (
    CapacityForm, CapacityFormSet, CustomAuthenticationForm, CustomUserCreationForm,
    ExcelUploadForm, LPUExcelUploadForm, LPUForm, NuevaTablaPagosForm, SLAReportForm,
    GenerarTablaSLAMesForm
)
from .models import (
    Capacity, LPU, NuevaTablaPagos, TablaDePagoCapacity, SLAReport, ZonaAdjudicacion,
    ZonaOperativa, PrecioEspecialidad, Especialidad
)
from .serializers import (
    CapacitySerializer, NuevaTablaPagosSerializer, TablaDePagoCapacitySerializer,
    SLAReportSerializer
)
from .utils import (
    mapear_especialidad, obtener_proyecto, clasificar_especialidad, limpiar_texto_especialidad
)
        ...
        if form.is_valid():
            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if not Capacity.objects.filter(
                        ...
                        Capacity.objects.create(
                            fecha_extract_registro=row['FECHA_EXTRACT_REGISTRO'],
                            nombre_tecnico=row['NOMBRE_TECNICO'],
                            nombre_team=row.get('NOMBRE_TEAM', ''),
                            especialidad_usuario=row.get('ESPECIALIDAD_USUARIO', ''),
                            area=row.get('AREA', ''),
                            rut_tecnico=row['RUT_TECNICO'],
                            zona_operacional=row['ZONA_OPERACIONAL'],
                            zona_adjudicacion=row['ZONA_ADJUDICACION'],
                            especialidad_team=row.get('ESPECIALIDAD_TEAM', ''),
                            capacidad=row.get('CAPACIDAD', 0),
                ...
                messages.success(request, f'Archivo Excel cargado correctamente. Nuevos registros: {nuevos_registros}, existentes: {registros_existentes}')
                return redirect('cargar_excel')
...
def ver_datos_capacity_mes(request, year, month):
    ...
    # Generar el rango elidido de la paginación
...
def crear_capacity(request):
    logger.debug("Ingresando a la vista crear_capacity")

    if request.method == 'POST':
        logger.debug("Se recibió una petición POST con datos: %s", request.POST)
        form = CapacityForm(request.POST)
        if form.is_valid():
            logger.debug("El formulario es válido. Guardando el nuevo registro.")
            form.save()
            logger.debug("Registro guardado correctamente, redirigiendo a dashboard_capacity")
            return redirect('dashboard_capacity')
        else:
            logger.debug("El formulario contiene errores: %s", form.errors)
    else:
        logger.debug("Se recibió una petición GET. Preparando formulario vacío.")
        form = CapacityForm()
    ...
    logger.debug("Opciones cargadas: nombre_team=%s, especialidad_usuario=%s, area=%s, zona_operacional=%s, especialidad_team=%s, zona_adjudicacion=%s",
                 list(nombre_team_choices), list(especialidad_usuario_choices), list(area_choices),
                 list(zona_operacional_choices), list(especialidad_team_choices), list(zona_adjudicacion_choices))
    ...
    logger.debug("Renderizando la plantilla pagos/crear_capacity.html con el contexto.")
...
def ver_todos_los_datos_capacity(request):
    ...
    # Generar rango elidido de paginación
    elided_range = paginator.get_elided_page_range(
        page_obj.number, on_each_side=2, on_ends=1
    )
...
def dashboard_lpu(request):
    # Obtiene los datos de LPU agrupados por año y mes de finalización
    lpu_data = (
        LPU.objects
        .values('ano_finalizacion', 'mes_finalizacion')  # Agrupa por año y mes
        .exclude(ano_finalizacion__isnull=True, mes_finalizacion__isnull=True)  # Excluye datos nulos
        .annotate(total_registros=Count('id'))  # Cuenta el total de registros
        .order_by('ano_finalizacion', 'mes_finalizacion')  # Ordena por año y mes
    )

    # Traduce el número del mes al nombre correspondiente
    for record in lpu_data:
        mes_numero = record['mes_finalizacion']
        try:
            record['mes_nombre'] = _(calendar.month_name[int(mes_numero)])
        except (ValueError, KeyError):
            record['mes_nombre'] = "Sin Mes"

    # Define el contexto con los datos de LPU
    context = {
        'lpu_data': lpu_data,
    }
...
@login_required
def ver_lpu(request):
    # Obtiene todos los registros de LPU, ordenados por 'id'
    lpu_list = LPU.objects.all().order_by('id')
    
    # Crea un paginador que divide los registros en páginas de 10 elementos
    paginator = Paginator(lpu_list, 10)
    ...
    # Calcula el rango elidido de páginas. Ajusta on_each_side y on_ends según necesites.
    elided_range = paginator.get_elided_page_range(
        page_obj.number, on_each_side=2, on_ends=1
    )
    
    # Renderiza la plantilla 'ver_lpu.html', pasando la página actual y el rango elidido
    return render(request, 'pagos/ver_lpu.html', {
        'page_obj': page_obj,
        'elided_range': elided_range
    })
...
def ver_lpu_por_mes(request, year, month):
    # Filtra los registros de LPU por el año y mes de finalización especificados
    lpu_data = LPU.objects.filter(ano_finalizacion=year, mes_finalizacion=month).order_by('id')
    
    # Crea un paginador que divide los resultados en páginas de 10 registros
    paginator = Paginator(lpu_data, 10)
    ...
    # Calcula el rango elidido de las páginas para mostrar en la plantilla
    elided_range = paginator.get_elided_page_range(
        page_obj.number, on_each_side=2, on_ends=1
    )
    ...
    context = {
        'page_obj': page_obj,
        'elided_range': elided_range,
        'mes_finalizacion': f"{year}-{month}"  # Ejemplo: 2024-12
    }
...
@login_required
def ver_lpu_por_ano(request, year):
    # Filtrar los registros de LPU por el año de finalización especificado
    lpu_data = LPU.objects.filter(ano_finalizacion=year).order_by('id')

    # Crear un paginador para dividir los resultados en páginas de 10 registros
    paginator = Paginator(lpu_data, 10)

    # Obtener el número de página actual de los parámetros de la solicitud GET
    page_number = request.GET.get('page')

    # Obtener el conjunto de objetos correspondientes a la página actual
    page_obj = paginator.get_page(page_number)

    # Calcular el rango elidido de las páginas para mostrar en la plantilla
    elided_range = paginator.get_elided_page_range(
        page_obj.number, on_each_side=2, on_ends=1
    )
    ...
    context = {
        'page_obj': page_obj,
        'elided_range': elided_range,
        'ano_finalizacion': year  # Pasar el año para mostrarlo en la plantilla
    }

    # Renderizar la plantilla 'lpu_por_ano.html' pasando el contexto
    return render(request, 'pagos/lpu_por_ano.html', context)
...
def editar_nuevatabladepagos(request, pk):
    logger.debug("Ingresando a la vista editar_nuevatabladepagos con pk=%s", pk)
    registro = get_object_or_404(NuevaTablaPagos, pk=pk)
    logger.debug("Se obtuvo el registro con ID %s: %s", pk, registro)

    if request.method == 'POST':
        logger.debug("Se recibió una petición POST con datos: %s", request.POST)
        form = NuevaTablaPagosForm(request.POST, instance=registro)
        if form.is_valid():
            logger.debug("El formulario es válido. Guardando cambios.")
            form.save()
            messages.success(request, 'Registro actualizado exitosamente.')
            logger.debug("Registro actualizado con éxito. Redirigiendo a nuevatabladepagos.")
            return redirect('nuevatabladepagos')
        else:
            logger.debug("El formulario contiene errores: %s", form.errors)
            messages.error(request, 'Por favor, corrija los errores en el formulario.')
    else:
        logger.debug("Se recibió una petición GET. Cargando formulario con datos del registro.")
    ...
    logger.debug(
        "Opciones cargadas: sociedad=%s, material=%s, especialidad=%s, zona_adjudicacion=%s, zona_operacional=%s, area=%s",
        list(sociedad_choices), 
        list(material_choices),
        list(especialidad_choices),
        list(zona_adjudicacion_choices),
        list(zona_operacional_choices),
        list(area_choices)
    )
    ...
    logger.debug("Renderizando la plantilla pagos/editar_nuevatabladepagos.html con el contexto.")
...
def crear_nuevatabladepagos(request):
    logger.debug("Ingresando a la vista crear_nuevatabladepagos.")

    if request.method == 'POST':
        logger.debug("Se recibió una petición POST con datos: %s", request.POST)
        # Crea una instancia del formulario con los datos enviados por el usuario
        form = NuevaTablaPagosForm(request.POST)
        logger.debug("Formulario inicializado: %s", form)

        if form.is_valid():
            logger.debug("El formulario es válido. Procediendo a guardar el registro.")
            # Guarda el nuevo registro en la base de datos
            form.save()
            # Muestra un mensaje de éxito al usuario
            messages.success(request, 'Registro creado exitosamente.')
            logger.debug("Registro guardado exitosamente. Redirigiendo a 'nuevatabladepagos'.")
        ...
        logger.debug("Se recibió una petición GET. Inicializando un formulario vacío.")
    ...
    logger.debug("Obteniendo opciones únicas para los campos desplegables.")
    sociedad_choices = NuevaTablaPagos.objects.values_list('sociedad', flat=True).distinct()
    material_choices = NuevaTablaPagos.objects.values_list('material', flat=True).distinct()
    especialidad_choices = NuevaTablaPagos.objects.values_list('especialidad', flat=True).distinct()
    zona_adjudicacion_choices = NuevaTablaPagos.objects.values_list('zona_adjudicacion', flat=True).distinct()
    ...
    logger.debug(
        "Opciones cargadas - Sociedad: %s, Material: %s, Especialidad: %s, Zona Adjudicacion: %s, Zona Operacional: %s, Área: %s",
        list(sociedad_choices),
        list(material_choices),
        list(especialidad_choices),
        list(zona_adjudicacion_choices),
    ...
    logger.debug("Renderizando la plantilla 'crear_nueva_tabla_pagos.html' con el contexto proporcionado.")
...
def generar_tabla_pago_capacity_mes(request, year, month):
    try:
        ...
        # Convertir Capacity a DataFrame
        df_capacity = pd.DataFrame(list(capacities.values()))
        logger.debug(f"Columnas de df_capacity: {df_capacity.columns}")
        logger.debug(f"Primeras filas de df_capacity:\n{df_capacity.head()}")

        # Verificar y normalizar columnas
        for col in ['especialidad_usuario', 'zona_adjudicacion', 'zona_operacional', 'area']:
            if col not in df_capacity.columns:
                logger.warning(f"La columna '{col}' no está presente en df_capacity. Se asignará un valor vacío.")
                df_capacity[col] = ''
            df_capacity[col] = df_capacity[col].fillna('').astype(str).str.strip().str.upper()

        # Obtener y normalizar NuevaTablaPagos
        df_nueva_tabla_pagos = pd.DataFrame(list(NuevaTablaPagos.objects.all().values()))
        logger.debug(f"Columnas de df_nueva_tabla_pagos: {df_nueva_tabla_pagos.columns}")

        for col in ['especialidad', 'zona_adjudicacion', 'zona_operacional', 'area']:
            if col not in df_nueva_tabla_pagos.columns:
                logger.warning(f"La columna '{col}' no está presente en df_nueva_tabla_pagos. Se asignará un valor vacío.")
                df_nueva_tabla_pagos[col] = ''
            df_nueva_tabla_pagos[col] = df_nueva_tabla_pagos[col].fillna('').astype(str).str.strip().str.upper()

        # Función para asignar proyectos
        def obtener_proyecto(nombre_team):
            # Reglas para asignar proyectos basados en el valor de nombre_team
            if 'COMANDO' in nombre_team:
                return 'COMANDO'
        ...
        # Función para clasificar si es un vehículo o una especialidad
        def clasificar_tipo(especialidad_usuario):
            palabras_vehiculo = ["VEHICULO", "PATNER", "CAMIONETA", "SERV VEHICULO"]
            if any(palabra in especialidad_usuario for palabra in palabras_vehiculo):
                return 'VEHICULO'
            return 'ESPECIALIDAD'

        # Asignar tipo_red y tipo_especialidad
        df_capacity['tipo_red'] = df_capacity['especialidad_usuario'].apply(asignar_tipo_red)
        df_capacity['tipo_especialidad'] = df_capacity['especialidad_usuario'].apply(clasificar_tipo)

        # Fusión de Capacity y NuevaTablaPagos
        df_merged = pd.merge(
            df_capacity,
            df_nueva_tabla_pagos,
            left_on=['especialidad_usuario', 'zona_adjudicacion', 'zona_operacional', 'area'],
        ...
        # Calcular Q y Total_a_Pago
        df_merged['Q'] = df_merged.apply(
            lambda row: Decimal(row['total_hh_phi']) / Decimal('176') if row['total_hh_phi'] else Decimal('0'),
            axis=1
        )
        df_merged['Total_a_Pago'] = df_merged.apply(
        ...
        # Asignar fecha
        df_merged['fecha'] = df_merged.get('fecha_extract_registro', date(year, month, 1))

        # Seleccionar columnas requeridas
        columnas_requeridas = [
            'nombre_tecnico', 'nombre_team', 'especialidad_usuario', 'rut_tecnico',
            'zona_operacional', 'zona_adjudicacion', 'total_hh', 'phi', 'total_hh_phi',
            'nuevo_precio', 'Q', 'Total_a_Pago', 'tipo_red', 'proyecto', 'tipo_especialidad', 'area', 'fecha'
        ]
        df_resumido = df_merged[columnas_requeridas].fillna(Decimal('0'))

        # Guardar registros en la base de datos con verificación manual de duplicados
        guardados, existentes = 0, 0

        for _, row in df_resumido.iterrows():
            existe = TablaDePagoCapacity.objects.filter(
                nombre_tecnico=row['nombre_tecnico'],
        ...
        logger.info(f"Registros guardados: {guardados}, Registros existentes: {existentes}")

        return render(request, 'pagos/tabla_pago_capacity_mes.html', {
            'datos': df_resumido.to_dict(orient='records'),
            'year': year,
            'month': month
        })
...
def ver_resumen_mensual(request, month, year):
    try:
        ...
        # Asegurar el tipo decimal de Total_a_Pago
        registros = registros.annotate(
            total_a_pago_cast=ExpressionWrapper(F('Total_a_Pago'), output_field=DecimalField())
        )

        # Agregaciones globales
        agregados = registros.aggregate(
            total_pago=Sum('total_a_pago_cast'),
            total_hh=Sum('total_hh'),
            total_q=Sum('Q')
        )

        # Crear resumen global con valores por defecto 0 si es None
        resumen = {
        ...
        # Resumen por área
        resumen_areas = list(registros.values('area').annotate(suma_pago=Sum('total_a_pago_cast')))
        total_areas = sum(item['suma_pago'] or 0 for item in resumen_areas)

        # Resumen por tipo de red
        resumen_tipo_red = list(registros.values('tipo_red').annotate(suma_pago=Sum('total_a_pago_cast')))
        total_tipo_red = sum(item['suma_pago'] or 0 for item in resumen_tipo_red)

        # Totales específicos para Móvil y Fija
        total_movil = sum(item['suma_pago'] for item in resumen_tipo_red if item['tipo_red'] == 'Móvil')
        total_fija = sum(item['suma_pago'] for item in resumen_tipo_red if item['tipo_red'] == 'Fija')

        # Resumen por zona operacional y tipo de red
        resumen_zona_tipo_red = list(registros.values('zona_operacional', 'tipo_red').annotate(
            suma_pago=Sum('total_a_pago_cast')
        ))
        total_zona_tipo_red = sum(item['suma_pago'] or 0 for item in resumen_zona_tipo_red)

        # Resumen por zona operacional y proyecto
        resumen_zona_proyecto = list(registros.values('zona_operacional', 'proyecto').annotate(
            suma_pago=Sum('total_a_pago_cast')
        ))
        total_zona_proyecto = sum(item['suma_pago'] or 0 for item in resumen_zona_proyecto)

        # Totales por proyecto
        totales_por_proyecto = registros.values('proyecto').annotate(
            suma_total=Sum('total_a_pago_cast')
        ).order_by('proyecto')
        total_por_tipo_proyecto = sum(item['suma_total'] or 0 for item in totales_por_proyecto)
        totales_por_proyecto_dict = {item['proyecto']: item['suma_total'] for item in totales_por_proyecto}

        # Resumen por tipo de especialidad
        resumen_tipo_especialidad = list(registros.values('zona_operacional', 'tipo_especialidad').annotate(
            suma_pago=Sum('total_a_pago_cast')
        ))
        total_tipo_especialidad = sum(item['suma_pago'] or 0 for item in resumen_tipo_especialidad)

        # Totales para Vehículo y Especialidad
        total_vehiculo = sum(item['suma_pago'] for item in resumen_tipo_especialidad if item['tipo_especialidad'] == 'Vehículo')
        total_especialidad = sum(item['suma_pago'] for item in resumen_tipo_especialidad if item['tipo_especialidad'] == 'Especialidad')

        # Limpieza y normalización de datos
        # Limpieza y normalización de datos para evitar errores
        for item in resumen_tipo_especialidad:
            tipo_especialidad = item.get('tipo_especialidad', '').strip().upper()  # Convertir a mayúsculas
            item['tipo_especialidad_normalizado'] = tipo_especialidad  # Para depuración

        # Totales para Vehículo y Especialidad
        total_vehiculo = sum(
            item['suma_pago'] or 0
            for item in resumen_tipo_especialidad
            if item.get('tipo_especialidad_normalizado') == 'VEHICULO'
        )
        total_especialidad = sum(
            item['suma_pago'] or 0
            for item in resumen_tipo_especialidad
            if item.get('tipo_especialidad_normalizado') == 'ESPECIALIDAD'
        )

        # Depuración para identificar problemas
        print("Resumen Tipo Especialidad:", resumen_tipo_especialidad)
        print("Total Vehículo:", total_vehiculo)
        print("Total Especialidad:", total_especialidad)
        ...
        return render(request, 'pagos/ver_resumen_mensual.html', {
            ...
            'resumen_tipo_red': resumen_tipo_red,
            'total_tipo_red': total_tipo_red,
            'total_movil': total_movil,
            'total_fija': total_fija,
            'resumen_zona_tipo_red': resumen_zona_tipo_red,
            'total_zona_tipo_red': total_zona_tipo_red,
            'resumen_zona_proyecto': resumen_zona_proyecto,
            'total_zona_proyecto': total_zona_proyecto,
            'totales_por_proyecto': totales_por_proyecto_dict,
            'total_por_tipo_proyecto': total_por_tipo_proyecto,
            'resumen_tipo_especialidad': resumen_tipo_especialidad,
            'total_tipo_especialidad': total_tipo_especialidad,
            'total_vehiculo': total_vehiculo,
            'total_especialidad': total_especialidad,
            'month': month,
            'year': year,
        })
